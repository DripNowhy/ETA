...
def wrap_and_generate(args, query, model, tokenizer, image=None, image_sizes=None):
    ...
    with torch.inference_mode():
        output_ids = model.generate(
            input_ids,
            images=image.to(dtype=torch.float16, device='cuda', non_blocking=True) if image is not None else None,
...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        tell_qs = TELL_PROMPT.format(cur_prompt, outputs[:1927])
        unsafe = wrap_and_generate(args, tell_qs, model, tokenizer, image=image_tensor, image_sizes=image_sizes)
        ...
        if unsafe.lower() in ["yes", 'yes.']:
            ...
            cap_qs = CAP_PROMPT.format(cur_prompt)
            caption = wrap_and_generate(args, cap_qs, model, tokenizer, image=image_tensor, image_sizes=image_sizes)

            ######################################### LLM-GEN-START #########################################

            gen_qs = LLM_GEN_PROMPT.format(caption, cur_prompt)
