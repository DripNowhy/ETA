...
def eval_model(args):
    ...
    for i, line in enumerate(tqdm(questions)):
        ...
        tell_qs = TELL_PROMPT.format(_question, outputs[:1927])
        unsafe = wrap_and_generate(args, tell_qs, model, tokenizer, image=images, image_sizes=image_sizes)
        ...
        if unsafe.lower() in ["yes", 'yes.']:
            ...
            cap_qs = CAP_PROMPT.format(_question)
            caption = wrap_and_generate(args, cap_qs, model, tokenizer, image=images, image_sizes=image_sizes)

            ######################################### LLM-GEN-START #########################################

            gen_qs = LLM_GEN_PROMPT.format(caption, _question)
