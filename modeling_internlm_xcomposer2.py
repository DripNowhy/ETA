...
class InternLMXComposer2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def forward(self,
                ...
                im_mask: Optional[torch.BoolTensor] = None,
    ...
    def _get_inputs_embeds(self, model, input_ids, text, image, need_bos=True,hd_num=24):
        pt1 = 0
        embeds = []
        im_mask = []
        img = None
        ...
        images_loc = [0]
        image_nums = len(image)
        if image_nums == 1 and text.find('<ImageHere>') == -1:
            #print ('auto append image at the begining')
            prompt = '<ImageHere>' + text
        else:
            prompt = text
        parts = prompt.split('<ImageHere>')
        wrap_embeds, wrap_im_mask = [], []
        ...
        for idx, part in enumerate(parts):
            ...
            if idx < image_nums:
                image_cur = image[idx]
                if type(image_cur) is str:
                    image_cur = Image.open(image_cur).convert('RGB')
                else:
                    image_cur = image_cur.convert('RGB')
                image_cur = Image_transform(image_cur, hd_num = 25)
                image_cur = self.vis_processor(image_cur).unsqueeze(0).to(self.device)
                img = self.encode_img(image_cur, hd_num)
                wrap_embeds.append(img)
                wrap_im_mask.append(torch.ones(img.shape[:2]))
                temp_len += img.shape[1]
        ...
        wrap_im_mask = wrap_im_mask[:, :self.max_length].to(self.device).bool()
        # if img is not None:
        #     return wrap_embeds, wrap_im_mask, img
        # else:
        return wrap_embeds, wrap_im_mask
